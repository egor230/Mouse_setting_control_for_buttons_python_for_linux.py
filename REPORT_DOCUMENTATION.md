...
## 20. Переработка эмуляции колёсика мыши: evdev/uinput вместо xdotool (по `analysis_and_improvements.md`)

> Дата: 2026-08-30. Применён рекомендуемый вариант из анализа — «вариант 3
> (UInput `REL_WHEEL`) + вариант 4 (событийная модель)». Цель — устранить
> слабые места W1–W9: форк процессов на каждый тик, «шторм» потоков,
> фиксированная скорость, дрейф таймингов, overscroll, задержка старта,
> лишняя bash-обёртка и отсутствие backpressure.

### 20.1 Старая реализация (что было)

`Job.run()` (поток автопрокрутки) в цикле каждые ~0.08с создавал **новый
дочерний поток** и из него вызывал `work_key.mouse_wheel_up()/mouse_wheel_donw()`,
которые через `subprocess.call(['bash','-c','xdotool click 4/5'])` форкали
ещё 2 процесса(bash + xdotool). Недостатки:

| ID | Проблема |
|----|----------|
| W1 | fork процесса на каждый «тик» → высокая нагрузка CPU, дрожание |
| W2 | потоки создаются и теряются без `join` → накопление под нагрузкой |
| W4/W5 | фиксированная скорость (`0.08+0.0115`с), дрейф таймингов |
| W6 | overscroll: после `pause()` успевал пройти 1–2 «лишних» клика |
| W7 | задержка старта ~80–90мс (сначала `time.sleep(0.08)`) |
| W8/W9 | бесполезная bash-обёртка, отсутствие backpressure |

### 20.2 Новая реализация (что стало)

**1. Скролл отправляется через ОТДЕЛЬНОЕ виртуальное устройство-указатель.**

`SmartTyper` держит два UInput-устройства:

- `self._ui` — виртуальная **клавиатура** (`EV_KEY` + `EV_LED`) — для клавиш и
  макросов;
- `self._ui_scroll` — виртуальный **указатель** `Mouse Setting Control Virtual
  Wheel` (`EV_KEY` (BTN_*) + `EV_REL` с `REL_X/REL_Y/REL_WHEEL/REL_HWHEEL`) —
  для колеса.

Почему так: X11 классифицирует UInput-устройство по capabilities. Запись
`EV_REL / REL_WHEEL` в устройство, объявленное клавиатурой (`EV_KEY`+`EV_LED`),
сервер **молча игнорирует** — скролла не происходит, хотя `write()` возвращает
успех (именно так выявился регресс «боковая не скроллит», подробности и
эмпирическое доказательство — в §21.1). Устройство с `EV_REL`+`EV_KEY` Xorg
регистрирует как **указатель** и превращает `REL_WHEEL` в настоящий скролл
(кнопки 4/5 + XI2 scroll valuator «Rel Vert Scroll»). Имя устройства содержит
`virtual`/`mouse`, поэтому фильтр `_find_keyboard()` не принимает его за
физическую клавиатуру.

```python
def scroll(self, direction, amount=1):
    # direction: +1 = вверх (REL_WHEEL +1), -1 = вниз (REL_WHEEL -1)
    # amount: сколько «щелчков» отправить за один SYN-пакет (мгновенный шаг на N строк)
    if self._ui_scroll is None:
        self._create_scroll_device()
    if self._ui_scroll is None:
        return False                     # /dev/uinput недоступен → fallback (xdotool)
    with self._lock:
...
Отправляет событие `EV_REL / REL_WHEEL` напрямую в ядро — **вообще без
процессов**, с минимальной задержкой; работает в Wayland и в играх.

**2. `Job.run()` переведён на событийную модель:**

- первый клик уходит **мгновенно** при `resume()` (устранена W7);
- автоповтор идёт по настраиваемому интервалу с компенсацией дрейфа
  (deadline-цикл через `time.monotonic()`, устранена W5);
- при `pause()`/`stop()` новых тиков не появляется — **нет overscroll** (W6);
- не создаются потоки на каждый тик и не форкается `xdotool` (W1/W2/W8/W9);
- для не-скролл слотов (клавиатурные) цикл прокрутки не запускается — поток
  просто ждёт (W3/6.A).

**3. Fallback.** Если `/dev/uinput` недоступен, `_emit()` возвращается к
прежнему `xdotool click 4/5` (по 1 команде на «щелчок», без bash-обёртки) —
поведение не «ломается», а мягко откатывается.

**4. Конфигурация скролла** (`mouse_scroll` — новый необязательный ключ JSON,
пер-профиль). Все параметры с **безопасными дефолтами** — отсутствие ключа
не меняет поведение и не требует правок UI:

```json
"mouse_scroll": {
  "<profile>": {
    "interval": 0.04,   // сек между тиками автоповтора (скорость)
    "amount": 1,        // «щелчков» колеса за один шаг (шаг)
    "invert": false,    // инверсия направления прокрутки
    "accel": false      // ускорение (рост шага) при длительном удержании
  }
}
```

### 20.3 Затронутые места

| Что | Где |
|-----|-----|
| `SmartTyper.scroll()` | `Pyqt6_libs_mouse.py`, класс `SmartTyper` |
| `Job.run()` + `_scroll_params/_emit/_repeat` | `Pyqt6_libs_mouse.py`, класс `Job` |
| импорт `pwd` | строка импортов (закрывает баг §17 «pwd без импорта», вариант 9.A) |
| `check_star()` | исправлен баг «сломанной защиты от второго запуска» (вариант 3.A: сравнивается `len(a)`, а не `len(process_list)`) |

### 20.4 Проверка

- `python3 -m py_compile Pyqt6_libs_mouse.py Pytq6_mouse_setting_control_for_buttons_for_linux.py` → `COMPILE OK`.
- Логика проверена изолированно (со стабами 3rd-party модулей): отправка
  `REL_WHEEL` с `amount`; мгновенный первый клик; **отсутствие overscroll**
  после `pause()`; трактовка конфига (`interval`/`amount`/`invert`); fallback
  на `xdotool` при недоступном uinput.
- Внешний вид GUI и все публичные API (`pause/resume/stop/set_sw/get_sw`,
  `set_hook_flag_mouse/get_hook_flag_mouse`, контракт `dispatch`) не менялись.
- Требуется ручная проверка пользователем: прокрутка боковой кнопкой в игре /
  браузере и при необходимости — прописать ключ `mouse_scroll` в профиль.

---

## 21. Сессия 2026-08-30 (2-я): исправление двух регрессий — прокрутка боковой кнопкой и ручной выбор профиля

> Пользователь сообщил: (1) в профиле по умолчанию боковая кнопка не
> прокручивает колесо; (2) после ручного выбора профиля в UI правая кнопка не
> нажимает `W`. Ограничения: не трогать внешний вид GUI и логику сохранения
> настроек. Правки — только в `Pyqt6_libs_mouse.py`.

### 21.1 Баг 1 — `REL_WHEEL` в клавиатурное UInput-устройство молча терялся (скролл не работал)

**Симптом.** Боковая кнопка (назначение `SCROLL_UP`/`SCROLL_DOWN`) в профиле
по умолчанию не скроллит.

**Диагностика (эмпирически, через `xinput test-xi2 --root`).** Два способа
эмуляции колеса сравнены по реальным событиям X11:

| Способ | Устройство в X11 | Результат |
|--------|------------------|-----------|
| `REL_WHEEL` в UInput (`EV_KEY`+`EV_LED`) — как было до фикса | клавиатура (`slave keyboard (3)`) | **событий колеса нет вообще** — `write()` «успешен», но события молча игнорируются |
| `REL_WHEEL` в UInput-указатель (`EV_KEY`+`EV_REL`) | **указатель** (`slave pointer (2)`, классы `XIScrollClass` vertical/horizontal, valuator «Rel Vert Scroll», increment 120) | `RawButtonPress detail:4 flags:emulated` → `ButtonPress 4` на core pointer — **реальный скролл** |
| `xdotool click 4` (XTEST, id=4) | core XTEST pointer | `ButtonPress/Release detail:4` — реальный скролл (старое поведение) |

**Корень.** `SmartTyper.scroll()` писал `REL_WHEEL` в то же устройство, что и
клавиатура (`self._ui`). X11 по capabilities объявляет такое устройство
клавиатурой и отбрасывает `EV_REL/REL_WHEEL`; т.к. `write()` не падал, fallback
на `xdotool` в `Job._emit()` не срабатывал → скролл молча не работал (старая
версия всегда шла через `xdotool click 4/5`).

**Исправление.** Добавлено второе UInput-устройство — **виртуальный указатель**
`Mouse Setting Control Virtual Wheel` (`_create_scroll_device()`, поле
`self._ui_scroll`): `EV_KEY` (BTN_*) + `EV_REL` (`REL_X/REL_Y/REL_WHEEL/
REL_HWHEEL`). `scroll()` пишет `REL_WHEEL` в него; клавиатурное устройство
осталось только для клавиш/макросов. Имя устройства содержит `virtual`/`mouse`
→ фильтр `_find_keyboard()` его не выбирает. Fallback на `xdotool click 4/5`
при недоступности `/dev/uinput` сохранён.

### 21.2 Баг 2 — ручной выбор профиля не применялся (правая кнопка не жмёт `W`)

**Симптом.** После клика по профилю (например, Far Cry с назначением «правая
кнопка = W») правая кнопка продолжает работать как обычная.

**Корень.** `MouseProfileRuntime.__init__` брал цель эмуляции из **активного
окна** (`self.game = check_current_active_window(...)`; см. §19.3 — там это
сделано намеренно, чтобы фоновый монитор не перезаписывал `current_app`). Но
при ручном клике по профилю активное окно — сам GUI (python-процесс, `.exe` в
дереве нет), поэтому `check_current_active_window` возвращал fallback
(`prev_game`/default) → runtime получал профиль по умолчанию (правая кнопка
оставалась `RBUTTON`), а выбранный профиль игнорировался.

**Исправление — приоритет цели эмуляции (развязка §19.3 сохранена):**

- `MouseProfileRuntime.__init__(store, start_game=None)`, порядок приоритета:
  1. `start_game` — явная цель (авто-переключение по активному окну из
     `emunator_mouse` либо явная цель из `prepare`);
  2. иначе `current_app` — профиль, выбранный в UI (**применяется сразу после
     клика по профилю**);
  3. иначе `check_current_active_window` (фолбэк).
  `current_app` по-прежнему **никогда** не перезаписывается монитором окна.
- `emunator_mouse` — переключение только по двум правилам:
  1. если в фокусе **реальное игровое окно** (`active_game != fallback`) — его
     профиль всегда главен;
  2. иначе (игрового окна в фокусе нет) — применяется выбранный в UI профиль
     (`current_app`), если он отличается от текущего.
  Раньше профиль менялся на любой `active_game` (включая fallback), поэтому
  ручной выбор мгновенно откатывался на дефолт на следующем проходе цикла.
- `prepare(store, start_game=None)` — передаёт цель в runtime; перезапуск после
  смены цели идёт через `prepare(store, restart_game)` (а не
  `start_startup_now`), так что `current_app` остаётся выбором пользователя, а
  не целью перезапуска.

**Эффект.** Клик по профилю сразу применяет его назначения (правая → `W`);
авто-переключение на окно реальной игры сохраняется; после ухода из игры
возвращается профиль, выбранный в UI, а не жёстко дефолтный.

### 21.3 Затронутые места

| Что | Где |
|-----|-----|
| `_create_scroll_device()` + `self._ui_scroll` | `Pyqt6_libs_mouse.py`, класс `SmartTyper` (~:512) |
| `SmartTyper.scroll()` → пишет в `_ui_scroll` | там же (~:661) |
| приоритет цели в `MouseProfileRuntime.__init__(store, start_game=None)` | там же (~:1344) |
| правила переключения в `emunator_mouse` + `restart_game` | там же (~:2252) |
| `prepare(store, start_game=None)` | там же (~:2377) |
| `Job._emit()` fallback на `xdotool` | без изменений (срабатывает, только если uinput недоступен) |

### 21.4 Проверка

- `python3 -m py_compile` обоих `.py` → синтаксис OK; и немодульный импорт GUI
  (`from Pytq6_mouse_setting_control_for_buttons_for_linux import
  evdev_key_to_label`) → OK.
- **E2E через реальный модуль** (venv `.../Project/myvenv`): 5×
  `smart_typer.scroll(1, 1)` → в `xinput test-xi2 --root` появляются 5 пар
  `RawButtonPress/Release detail:4 flags:emulated` от нового устройства
  (в X — `slave pointer`); резолюция профилей: default → `C:/Windows/explorer.exe`;
  `current_app` = Far Cry → Far Cry; `start_game` переопределяет выбор.
- Внешний вид GUI и логика сохранения/загрузки настроек не менялись.
- Временные uinput-устройства теста исчезают при закрытии fd (после теста
  `xinput list` не показывает виртуальных устройств).
- Осталось ручное подтверждение пользователем в интерактивном GUI: скролл
  боковой кнопкой и правая кнопка → `W` после клика по профилю игры.

---

## 22. Сессия 2026-09-01: баг переключения профилей — race condition runtime

### 22.1 Баг

**Симптом.** Когда игра закрывается, профиль не возвращается на «По умолчанию»
(`explorer.exe`), даже если кликнуть по нему мышкой. Правая кнопка продолжает
отдавать `W` (или вообще ничего не делает).

**Корень — три взаимосвязанные причины:**

1. **Race condition между `emunator_mouse` и статусом `current_app`.** Монитор
   окна крутится в отдельном потоке. Когда игра закрывается,
   `check_current_active_window` возвращает fallback → старый `emunator_mouse`
   выходит по `break` с `restart_game = target` и **сам перезапускает**
   `prepare(...)` в новом потоке. Если в этот момент `current_app` ещё хранит
   путь закрытой игры (а `check_label_changed` не успел обновить его), то новый
   runtime снова выбирает старую игру — профиль «По умолчанию» не применяется.

2. **Устаревший runtime перетирал новый.** Старый `emunator_mouse` в `finally`
   решал: если `was_stopped_externally == False` и есть `restart_game` — он
   перезапускается. Т.е. поток, уже сменённый ручным кликом, мог «догнать» и
   перезапуститься позже, затерев корректный runtime от клика. Несколько
   потоков создавали runtime одновременно.

3. **Устаревший listener обрабатывал события.** `on_click` не проверял,
   актуален ли ещё его runtime.

### 22.2 Исправление — атомарная смена runtime + приоритет ручного выбора

В `MouseSettingAppMethods.__init__` добавлены:
- `self._runtime_lock = threading.RLock()` — защита `_active_runtime` и счётчика
  поколений (RLock, т.к. `prepare` держит lock и вызывает `_stop_active_runtime`,
  который тоже берёт lock — без реентерабельности был бы дедлок);
- `self._runtime_generation = 0` — счётчик поколений runtime.

Изменённые методы:

| Метод | Изменение |
|-------|-----------|
| `MouseProfileRuntime.__init__(store, start_game=None, force_ui_selection=False)` | Новый `force_ui_selection` — приоритет №0: ручной выбор из UI применяется немедленно (даже если `current_app` ещё указывает на игру); дальше прежние шаги (start_game → current_app → активное окно). |
| `prepare(store, start_game=None, force_ui_selection=False)` | Под lock: `_stop_active_runtime`, `++generation`, создание runtime, `apply_button_map`, `set_current_path_game`, `_active_runtime = runtime`. Поток запускается с `(runtime, my_generation)`. |
| `_stop_active_runtime(store)` | Всё под `_runtime_lock` (runtime.stop + `_active_runtime = None`) + `store.set_thread(0)`. |
| `emunator_mouse(runtime, generation)` | Под lock проверяет, что `_active_runtime is runtime` **и** `_runtime_generation == generation`; если устарел — тихо выходит. `on_click` тоже проверяет актуальность. В `finally` решает о «перезапуске» только если `is_same_generation` и не остановлен извне; перезапуск — прямой вызов `self.prepare(store, restart_game)` из этого же фонового потока (под `_runtime_lock`, атомарно). |
| `start_startup_now(store=None, force_ui_selection=False)` | Пробрасывает `force_ui_selection` в `prepare`. |
| `check_label_changed(count)` | Передаёт `start_startup_now(..., force_ui_selection=True)` — ручной клик по профилю применяется сейчас, с приоритетом №0. |

**Эффект:** в любой момент активен ровно один runtime; устаревший поток не
перетирает новый; ручной выбор профиля (в т.ч. «По умолчанию») применяется
немедленно после клика.

### 22.3 Проверка

- `python3 -m py_compile` трёх `.py` (data/libs/GUI) → OK.
- `pyflakes` → нет жёстких ошибок (`undefined name`/синтаксис).
- Модульный тест приоритета `MouseProfileRuntime` (venv): 8 проверок — `start_game`
  > `current_app`; `force_ui_selection=True` = приоритет №0 (в т.ч. ручной выбор
  дефолта поверх активного игрового окна); при невалидном `current_app` —
  валидное переопределение; фолбэк активного окна; RLock реентерабелен → все OK.
- Импорт модулей (data/libs) + интроспекция новых сигнатур → OK.
- Осталось ручное подтверждение в интерактивном GUI: закрыть игру → профиль
  возвращается на выбранный в UI; клик по «По умолчанию» → правая кнопка
  становится обычной.

### 22.4 РЕГРЕССИЯ, найденная при тесте игры (Portal Stories Mel) 2026-09-01

**Симптом.** Запуск игры через wine-скрипт (`portal2.exe`) → профиль игры
**больше не подхватывается автоматически**, раньше всегда переключался.

**Корень.** В §22.2 перезапуск рантайма при смене профиля делался через
`QTimer.singleShot(0, lambda: self.prepare(...))`. Но этот вызов исполняется в
**фоновом потоке `emunator_mouse`**, у которого **нет Qt event loop**:
`QTimer.singleShot(0, ...)` создаёт таймер в текущем (фоновом) потоке, и из-за
отсутствия его event loop **таймер никогда не срабатывает**
(подтверждено тестом: `fired == False`). В результате рантайм ставил
`restart_game`, выходил, но `prepare` не вызывался → `_active_runtime` оставался
со старым (дефолтным) профилем → авто-переключение на игру не происходило.

Регрессия появилась именно в §22, т.к. до неё перезапуск выполнялся другим
способом (без «мёртвого» QTimer из фонового потока).

**Исправление.** Заменить `QTimer.singleShot(0, prepare(...))` на **прямой вызов**
`self.prepare(runtime.store, restart_game)` в том же фоновом потоке. Это
безопасно: `prepare` атомарен под `_runtime_lock`, не содержит GUI-вызовов и сам
запускает новый listener-поток. Генерационный guard из §22.2 сохранён (перезапуск
только если `is_same_generation` и не остановлен извне).

**Проверка.** Автономный тест (venv, QCoreApplication): подготовка runtime с
дефолтным профилем → имитация активного окна игры →
`AUTO-SWITCH OK: runtime.game = .../portal2.exe` (gen 1→2). Реверс (возврат к
дефолту) — по тем же правилам emunator.

---

## 23. Сессия 2026-09-05: скрипты боковых кнопок, single-instance, подсветка, фикс авто-переключения

### 23.1 Баг: скрипт боковой кнопки мыши не сохранялся и не работал

**Симптом.** Пользователь назначает bash-скрипт на боковую кнопку мыши
(`XBUTTON1`/`XBUTTON2`) через редактор (`mouse_scrpt_keyboard_with_editor`),
закрывает редактор — назначение «не держится»: скрипт не применяется, а после
перезапуска не виден.

**Корень.** Функция очистки `cleanup_empty_script_entries` (`Pyqt6_libs_data.py`)
безусловно удаляла скрипты боковых кнопок `XBUTTON1`/`XBUTTON2`, если в
`key_value[профиль][5]`/`[6]` было **непустое** значение. Слоты 5 и 6 по
умолчанию заполнены `SCROLL_UP`/`SCROLL_DOWN` (непустые почти всегда), поэтому
сохранённый через редактор скрипт стирался сразу же (`kill_notebook` → `save_jnson`
руками вызывает `cleanup_empty_script_entries`). Реальное конфликтное очищение
ключ/скрипт уже делается точечно в `update_button` (только когда пользователь
выбирает клавишу), поэтому дублирующий блок был удалён.

**Исправление.** В `cleanup_empty_script_entries` убран блок, который вычищал
XBUTTON-скрипты по `key_value`. Проверено: после фикса скрипт боковой кнопки
сохраняется и доступен живьём, пустые скрипты по-прежнему удаляются.

### 23.2 Закрытие редактора скрипта: всегда сохранять без запроса

**Поведение (уточнено пользователем).** При закрытии редактора скрипта
(клавиатурного или мышиного) **никакого запроса не показывается** — редактор
**всегда сохраняет** введённый скрипт в память (`kill_notebook` → `save_jnson`).
Запрос «Хотите сохранить изменения?» остаётся **только при закрытии главного
окна программы** (`closeEvent`, если `DeepDiff(old_data, new_data)` непустой).

**Мгновенное применение.** Скрипт из редактора вступает в силу **сразу после
закрытия**, без перезапуска программы: и мышиный runtime (`_mouse_script`), и
клавиатурный слушатель (`handle_keyboard_macro`) читают скрипты **живьём** из
`dict_save.return_jnson()` при каждом нажатии. Для клавиатурных макросов после
сохранения снова показывается основная клавиатура и обновляется подсветка
(`update_keyboard_display`).

### 23.3 Мгновенное применение скрипта после создания

**Симптом.** Хочется, чтобы скрипт начал работать сразу после создания, без
закрытия приложения/перезапуска эмуляции.

**Статус.** Это уже обеспечено архитектурой: runtime читает скрипт **живьём**
из `return_jnson()` при каждом нажатии (`_mouse_script` →
`script_mouse[current_game][button_name]`), а `kill_notebook` пишет в память
после «Сохранить». После §23.1 (скрипт больше не стирается очисткой) назначение
вступает в силу немедленно после сохранения в редакторе — дополнительных
действий не требуется. Запись в файл происходит при закрытии главного окна
(как и остальные настройки).

### 23.4 Запрет запуска второго экземпляра

**Добавлено.** Защита single-instance:
- lock-файл `/tmp/mouse_setting_control.lock` с PID процесса;
- `acquire_single_instance_lock()` — если lock держит **живой** PID
  (`/proc/<pid>` существует) → второй экземпляр показывает
  «Программа уже запущена» и выходит;
- если PID мёртв (крэш/`os._exit`) → lock считается устаревшим и занимается;
- `release_single_instance_lock()` вызывается в `closeEvent` перед `os._exit(0)`;
  при Cancel закрытия lock не снимается (процесс продолжает работать).

### 23.5 Визуальная подсветка назначенных скриптов (серая кнопка)

**Добавлено (запрос пользователя).** Явно видно, на какие кнопки мыши уже
назначен скрипт, а на какие — нет:

- **серый цвет** кнопки скрипта = для этой кнопки текущего профиля сохранён
  непустой скрипт (есть запись `script_mouse[current_app][кнопка]`);
- **обычный цвет** = скрипта нет.

**Реализация.** Из блока в `check_label_changed` выделен метод
`MouseSettingAppMethods.update_script_button_colors()`. Его вызовы:
- `check_label_changed` — при выборе профиля списком (серым/обычным для выбранной игры);
- `kill_notebook` — сразу после сохранения редактора: назначил скрипт → кнопка
  становится серой; стёр текст (`#!/bin/bash` не считается) → возвращается обычный цвет;
- `update_button` — при выборе клавиши в выпадающем списке (скрипт слота
  удаляется) → кнопка снова обычного цвета;
- `start_app` — корректная подсветка с первой секунды работы программы.

Метод не трогает сохранённые данные; только читает `script_mouse` для текущего
`current_app` и раскрашивает `self.buttons_script[i]`.

### 23.6 Баг: скрипт боковой кнопки не выполнялся при авто-переключении профиля (Квантум Брейк)

**Симптом (сообщил пользователь).** При **ручном** выборе профиля игры
(например «Квантум Брейк») боковая кнопка выполняет скрипт. При
**автоматическом** переключении на ту же игру (главное окно стало игрой)
правая кнопка работает, а скрипт боковой кнопки **не выполняется**.

**Диагностика.** Путь диспетчера: `MouseProfileRuntime.dispatch()` →
`self._mouse_script(binding.slot)` → `script_mouse[current_game][кнопка]`.
Различие между ручным и авто-путем — **ключ, по которому ищется скрипт**:

- `MouseProfileRuntime.game` — профиль, который рантайм **реально эмулирует**
  (при авто-переключении равен пути игры из `check_current_active_window`);
- `jnson["current_app"]` (`store.get_cur_app()`) — профиль, выбранный в **UI**.

Авто-переключение (`emunator_mouse` → `prepare(start_game=...)`) по дизайну
**не перезаписывает** `current_app` (фикс §19.3). Значит скрипт искался по
`current_app` = «По умолчанию», а не по активному профилю игры. Правая кнопка
в это время работала, потому что `assignments = key_value[game]` берётся из
`self.game` — отсюда «RMB ок, бока не ок».

Сценарии, дававшие баг/работавшие:
- ручной выбор QB: `current_app == game == QB` → скрипт находился;
- авто QB: `game == QB`, `current_app == DEFAULT` → скрипт **не находился**.

Воспроизведено юнит-тестом (`MouseProfileRuntime.__new__` + `_mouse_script(5)`):
при `game=QB, current_app=DEFAULT` возвращалось `''`.

**Исправление.** `_mouse_script(slot)` теперь берёт **`self.game`** (активный
профиль рантайма), а не `self.store.get_cur_app()`:
`script_mouse[self.game][defaut_list_mouse_buttons[slot]]`.

**Проверка фикса** — тест `/tmp/opencode/test_mouse_script_key.py`:
1. авто-переключение (`game=QB`, `current_app=DEFAULT`) → скрипт найден ✓;
2. ручной выбор (`game=QB`, `current_app=QB`) → скрипт найден ✓;
3. дефолтный профиль (`game=DEFAULT`, без скрипта) → скрипт не выполняется ✓;
4. после закрытия игры фолбэк (`game=DEFAULT`, `current_app=QB`) → скрипт QB
   не выполняется (по активному профилю) ✓.

Это же дерево чтения используется в `handle_keyboard_macro` для клавиатурных
макросов; `kill_notebook`/редакторы по-прежнему пишут скрипты под
`current_app` — это корректно, потому что сохранение всегда идёт для выбранной
в UI игры.

### 23.7 Проверка

- `py_compile` трёх `.py` → OK.
- Юнит-тест `_mouse_script` (self.game): авто/ручной/дефолт/фолбэк → OK.
- Юнит-тест сохранения боковой кнопки: скрипт `XBUTTON1` переживает
  `cleanup_empty_script_entries`, `_mouse_script(5)` отдаёт свежий текст → OK.
- Юнит-тест `kill_notebook` (всегда-сохранять): закрытие редактора записывает
  скрипт в память и закрывает окно без какого-либо запроса → OK.
- Юнит-тест single-instance: первый acquire True, второй False, после release
  снова True → OK.
- Юнит-тест подсветки: без скриптов — все кнопки обычные; назначили скрипт на
  `XBUTTON1` → только она серая; другой профиль без скриптов — все обычные;
...
## 27. Канонизация путей pid по директориям профилей (Portal Reloaded / GTAVLauncher и др.)

**Дата:** 2026-09-11

**Симптом.** Игра запущена, но профиль не подхватывается: окно принадлежит
процессу `portal2.exe` (PID с ppid=1, без лаунчера в дереве), а в настройках
профиль хранится как `Play Portal 2.exe` — лаунчер из той же папки. Подстрочное
сравнение (`is_path_in_list`) совпадения не даёт → профиль «Portal Reloaded»
не находится. Аналогичный сценарий: GTA V (`GTAVLauncher.exe` в дереве окна vs
`GTA5.exe` в JSON), Far Cry (`Bin32/FarCry_WinSV.exe` vs `Bin32/FarCry.exe`).

**Решение (минимальная добавка, старая логика не тронута).** В
`check_current_active_window` сразу после `Get_pid_and_path_window()` значения
словаря `{pid: путь}` приводятся к «каноническим» путям из настроек:

- `build_profile_dir_map(games_checkmark_paths, force=False)` — строит карту
  `{exe (lower) -> путь профиля}` по всем папкам профилей с галочкой:
  сканирует exe-файлы **в самой папке профиля** (без подпапок — Bin32-стиль
  профилей работает, потому что путь профиля уже указывает внутрь Bin32);
  результат кэшируется на атрибуте функции (ключ — кортеж путей с галочками,
  TTL 1 час) — цикл проверки крутится каждые ~30 мс, без кэша NTFS-диск
  молотился бы постоянно;
- `canonicalize_pid_paths(data_dict, games_checkmark_paths)` — для каждого pid:
  если его путь есть в карте → заменить на путь профиля; если путь лежит
  в директории какого-то профиля, но в карте отсутствует (exe появился позже
  построения) → форс-пересборка карты и повторная проверка.

**Правило: одна директория из настроек = одна игра.** Любой exe из папки
профиля приравнивается к пути профиля.

**Учтённые нюансы:**
1. Два профиля в одной папке — выигрывает первый по порядку списка
   (`if full not in exe_map`).
2. «Общая» папка с кучей чужих exe (>25, напр. `linux must have/` с
   `total_commander.exe`) — маппится ТОЛЬКО сам exe профиля, иначе любой
   соседний exe (bash-диагностика, файловый менеджер) захватил бы чужой
   профиль (аналог бага-слоя-1 из §26).
3. Недоступные/несуществующие папки (в т.ч. Windows-пути `C:/...` дефолтного
   профиля) пропускаются молча (`OSError`).
4. Не-exe значения словаря и посторонние pid не трогаются.
5. Формат ключа карты — `normpath().lower()`, чтобы `Portal 2.exe` и
   регистр/слэши не ломали совпадение.
6. Прямое совпадение (путь == путь профиля) после канонизации сохраняется —
   значение просто приравнивается к самому себе.

**Следствие.** Совпадение с профилем теперь происходит в основной ветке
сравнения ДО ветки PortProton — сломанный scoping-код §26 (`_is_game`/
`_walk_up`/`expanded`/`game_path`/`win_pid_int` из локальной области
`Get_pid_and_path_window`) просто перестаёт достигаться в сценарии
«exe из папки профиля». Сама мёртвая ветка НЕ чинилась (по решению
пользователя — коренных изменений не делаем).

**Проверка.**
- `py_compile` → OK.
- Изолированный тест (реальные папки игр, 7 сценариев): GTA (лаунчер в
  дереве → `GTA5.exe` профиля), Portal Reloaded (`portal2.exe` →
  `Play Portal 2.exe`), прямое совпадение, Far Cry Bin32
  (`FarCry_WinSV.exe` → `FarCry.exe`), `total_commander.exe` в общей папке,
  100 вызовов из кэша за 0.000 с, форс-пересборка при новом exe → ALL PASSED.
- Осталось ручное подтверждение пользователем: запустить Portal Reloaded →
  профиль подхватывается сам, без ручного клика.

---

## 28. Сессия 2026-09-12: ключ `additional_exe`, режим «Повторить», размер окна

**Дата:** 2026-09-12

### 28.1 Новый ключ JSON `additional_exe` — все exe директорий профилей

**Задача.** Зафиксировать в настройках полные пути всех `.exe`, лежащих в
верхней папке пути каждого профиля (без подпапок), чтобы не пересканировать
директории на каждый чих и не «терять» exe, соседствующие с exe профиля.

**Реализация (`Pyqt6_libs_data.py`):**
- `scan_profile_dir_exes(profile_path)` (строка 327) — отсортированный список
  полных путей всех `*.exe` из `os.path.dirname(profile_path)` (exe профиля
  входит сам); для недоступных папок (напр. дефолтный Windows-профиль
  `C:/Windows/...`, в Linux не смонтирован) и для пустых списков — возвращает
  просто `[profile_path]`.
- `ensure_additional_exe(res)` (строка 345) — `res.setdefault('additional_exe', {})`:
  для каждого профиля из `res['paths']`, у которого ключа ещё нет, пишет
  результат `scan_profile_dir_exes(profile)`. Существующие ключи не трогает.

**Точки вызова:**
- Старт приложения — оба пути загрузки JSON: `Pytq6_mouse_setting_control_for_buttons_for_linux.py:24` и `:35`;
  вызов идёт ДО `save_old_data()`, поэтому ложный диалог «Сохранить изменения?»
  не всплывает. Заполнение идёт в ПАМЯТИ; на диск ключ попадает по штатной схеме
  проекта — при сохранении пользователем (диалог в `closeEvent`).
- Добавление новой игры — `Pyqt6_libs_mouse.py:1875`, сразу после
  `insert_profile_after`.

**Проверка.** Offscreen-смоук: для 23 профилей в памяти сформирован
`additional_exe`; напр. Far Cry — 9 exe из папки профиля (Bin32-стиль).

### 28.2 Режим «Повторить» (checkbox + длительность) для клавиш, кнопок и прокрутки

**UI (`Pytq6_mouse_setting_control_for_buttons_for_linux.py:276-286`).** В каждый
из 7 рядов добавлены:
- чекбокс «Повторить» → `self.mouse_repeat_check_buttons` (инициализация :14);
- комбо длительности `["", "5", "10", "20", "30"]` → `self.mouse_hold_duration_combos` (:15).

**JSON-структура (на игру):** `mouse_repeat` (список из 7 bool) и
`mouse_hold_repeat_seconds` (7 int|null). Whitelist — модульная константа
`allowed_repeat_durations = (5, 10, 20, 30)` (`Pyqt6_libs_mouse.py:28`).
Пишут `update_mouse_repeat` (:2809) и `update_mouse_hold_duration` (:2832);
при снятии галочки пустой список убирается из словаря (мусора нет).
Допустимость значения — через `_slot_duration` (:1512): значение вне whitelist
трактуется как None. Загрузка в UI — `Pyqt6_libs_mouse.py:2536-2543`.

**Взаимоисключение с «Держать нажатой»:** включение «держать» снимает
«повторить» и наоборот — `check_mouse_press_button` (:2785)
и `update_mouse_repeat` (:2809) через `blockSignals` + очистку
противоположного JSON-флага. Комбинация невозможна по построению.

**Runtime-семантика (`_handle_mouse_action` ~1522, `_handle_keyboard_action` ~1630):**
- С выбранной длительностью — циклический повтор ровно N секунд.
- Без длительности (комбо пустое/невалидное) — **бесконечный повтор до
  повторного нажатия той же кнопки** (toggle по `_timed_holds[slot]`).
- **Скрипт приоритетнее кнопки:** если на слот привязан bash-скрипт — цикл
  выполняет ТОЛЬКО скрипт (без эмуляции клавиши/кнопки); иначе — импульсы
  press/release каждые ~0,05 с.
- Прокрутка: с длительностью — `resume` скроллера + `_pause_worker_after`;
  без — непрерывная прокрутка до повторного нажатия.
- `finally` в цикле сбрасывает `_timed_holds[slot]` и делает страховочный
  release (только в не-скриптовой ветке). Отпускание физической кнопки цикл
  не прерывает.

**Новые функции:** `_repeat_key_loop(slot, key_value, script=None, total_duration=None)`
(:1708), `_repeat_mouse_loop(slot, target_button, script=None, total_duration=None)`
(:1731); `total_duration=None` = цикл до toggle-off.

**Проверка.** Поведенческие тесты со стабами вместо устройств: повтор до
toggle-off, «только скрипт» (0 событий клавиатуры/мыши), автозавершение по
длительности — все PASS; регресс data-тестов — без изменений; `py_compile`
трёх файлов — OK.

### 28.3 Размер окна (−5% после +20%)

Запрос: сначала шире на +20%, затем ещё раз на −5%. Арифметика:
910 → 1092 → 1037 (`setGeometry`) / 940 → 1128 → 1072 (`setFixedSize`).
Текущее значение в файле: `setGeometry(400, 340, 1037, 386)`,
`setFixedSize(1042, 346)` (`Pytq6_mouse_setting_control_for_buttons_for_linux.py:197-198`);
1042 — ручная подгонка фиксированной ширины после смоук-теста (в момент теста
окно открывалось 1072).
