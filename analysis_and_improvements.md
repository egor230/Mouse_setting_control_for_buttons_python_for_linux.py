...
> Статус на 2026-09-12: ✅ внедрено · ⏳ открыто. Подробности — в
> `REPORT_DOCUMENTATION.md` §20 (переработка колёсика), §21 (фиксы регрессий
> прокрутки и ручного выбора профиля), §24 (сохранение порядка профилей),
> §25 (положение нового профиля + скачок списка при двойном клике),
> §26 (ветка PortProton в check_current_active_window — захват
> постороннего профиля через подстроку), §28 (ключ `additional_exe`,
> режим «Повторить», размер окна).
...
| **3** | `check_star` сломан | среднее (двойной запуск) | низкая | 3.A или 3.B | ✅ внедрено (3.A + lock-файл, см. REPORT §20.3, §23.4) |
...
| **11** | `reorder_keys_in_dict` возвращал `None` (мёртвый код из-за отступа) → `move_element` стирал настройки | высокое (перемещение профилей) | тривиально | фикс отступа | ✅ внедрено 2026-09-10 (REPORT §24.2) |
| **12** | DeepDiff не видит порядок ключей `paths` → перестановка профилей не попадала в диалог сохранения | высокое (порядок не сохранялся) | низкая | отдельная проверка порядка | ✅ внедрено 2026-09-10: `profile_order_changed` в closeEvent (REPORT §24.3) |
| **13** | новый профиль падал в конец списка вместо позиции «под активным» | среднее (UX) | низкая | вставка после якоря | ✅ внедрено 2026-09-10: `insert_profile_after` (REPORT §25.1) |
| **14** | двойной клик (переименование) — список «убегал» вниз-вверх (eventFilter возвращал устаревший `_scroll_lock_val` на release) | среднее (UX) | низкая | запоминать позицию на каждом событии, release не возвращает сам | ✅ внедрено 2026-09-10 (REPORT §25.2) |
| **15** | ветка PortProton в `check_current_active_window` хватала ВСЕ процессы при `has_portproton=True` (всегда True из-за фоновых services.exe) и возвращала первый попавшийся профиль — ручной выбор профиля сбрасывался на 30 мс | высокое (переключение профилей) | низкая | gate: проверять _is_game/_walk_up перед перебором | ✅ внедрено 2026-09-10 (REPORT §26) |
| **16** | список всех `.exe` директории профиля не хранился в настройках — при добавлении игры приходилось бы пересканировать папку заново | среднее (скорость/полнота детекта) | низкая | ключ `additional_exe` + `scan_profile_dir_exes`/`ensure_additional_exe` при старте и добавлении игры | ✅ внедрено 2026-09-12 (REPORT §28.1) |
| **17** | не было циклического повтора клавиш/кнопок/прокрутки (нужно для «строительных» макросов) | среднее (UX/функциональность) | средняя | «Повторить» + длительность 5/10/20/30 сек; без длительности — бесконечно до повторного нажатия; привязанный скрипт приоритетнее кнопки; взаимоисключение с «Держать» | ✅ внедрено 2026-09-12 (REPORT §28.2) |
| **18** | ширина окна настроек мала для новых колонок (Повторить/длительность) | низкое (UX) | тривиально | +20% ширины, затем −5% | ✅ внедрено 2026-09-12: 1037/1042 (REPORT §28.3) |

> **Эксплуатационная заметка (из §24.2):** проект живёт на внешнем диске
> (`/mnt/...`, NTFS/exFAT) — mtime имеет грубую гранулярность, и
> `__pycache__` может отдавать УСТАРЕВШИЕ .pyc после правок. После изменения
> `.py`-файлов удаляйте `__pycache__` (или запускайте с
...
Одна из критичных проблем — **is_path_in_list** сравнивает через подстроку
(`path in profile_path`), что даёт ложные срабатывания: процесс с
cwd/аргументом, содержащим часть пути профиля (например, `.../linux must
have/`), матчится с чужим профилем. Ветка PortProton была наиболее
агрессивным пользователем этой функции (перебор всех процессов при
`has_portproton=True`), но аналогичные проблемы возможны и в других местах,
где используется `is_path_in_list`. Решение в §26 — gate на дерево окна;
полное решение — замена подстрочного сравнения на более точный match
(basename / конец пути).

**Обновление 2026-09-10 (третья волна, помимо колёсика):** устранены баги
списка профилей — (1) `reorder_keys_in_dict` возвращал `None` из-за мёртвого
кода (лишний отступ) и «Вверх/Вниз» стирали настройки; (2) DeepDiff не видит
порядок ключей `paths`, из-за чего перестановка профилей не попадала в диалог
«Сохранить изменения?» — добавлена проверка `profile_order_changed`;
(3) новый профиль теперь встаёт сразу ПОД активным (`insert_profile_after`),
а не в конец списка; (4) устранён «скачок» списка при двойном клике
(переименовании) — eventFilter больше не возвращает устаревшую позицию
скролла на release. Детали и проверки — `REPORT_DOCUMENTATION.md` §24, §25.

**Обновление 2026-09-12 (четвёртая волна):**
- добавлен ключ JSON `additional_exe` — для каждого профиля список полных путей
  всех `.exe` из верхней папки пути профиля (без подпапок). Дозаполняется в
  памяти при старте (`ensure_additional_exe` до `save_old_data()` — без ложного
  диалога сохранения) и сканируется при добавлении новой игры. На диск попадает
  при штатном сохранении (REPORT §28.1);
- режим «Повторить» (checkbox + комбо 5/10/20/30 сек) для клавиш, кнопок мыши и
  прокрутки: с длительностью — ровно N секунд, без неё — бесконечный повтор до
  повторного нажатия (toggle). Взаимоисключение с «Держать нажатой». Если на
  слот привязан bash-скрипт — повторяется ТОЛЬКО скрипт, а не кнопка/клавиша
  (REPORT §28.2);
- окно настроек расширено (+20% ширины, затем −5%): итог 1037 (`setGeometry`) /
  1042 (`setFixedSize`) (REPORT §28.3).

Детали и проверки — `REPORT_DOCUMENTATION.md` §28.
